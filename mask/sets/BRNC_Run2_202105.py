...
from mask.tools import PlotterLib, ContactExporter
...
def main(args):
    ...
    probe_points = ContactExporter(lib)

    probe_points.include("DIODE_.*_[0-9]+")
    probe_points.rename("DIODE_2mm_", "F")
    probe_points.rename("DIODE_4mm_", "E")
    probe_points.rename("DIODE_6mm_", "D")
    probe_points.rename("DIODE_8mm_", "C")
    probe_points.rename("DIODE_12mm_", "B")

    probe_points.contacts("[FE][0-9]+", {"": [-1000, 1000]})
    probe_points.contacts("[DC][0-9]+", {"": [-2000, 2000]})
    probe_points.contacts("B[0-9]+", {"": [-3000, 3000]})

    probe_points.reference("B0")
    # Emulate some natural sort by 0 pre-padding the diode number
    probe_points.sort(lambda item: (item[0], '%5i'%(int(item[1:]))))

    probe_points.generate()
